...
func Times(config *c.Config) *chi.Mux {
	r := chi.NewRouter()
	r.Get("/", getWeather(config))

	return r
}

func getWeather(config *c.Config) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		var t Time
		timeUri := "http://" + config.TIME_URI + "/api/json/utc/now"
